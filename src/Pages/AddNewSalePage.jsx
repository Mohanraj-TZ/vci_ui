...
export default function AddNewSalePage() {
    ...
    if (['quantity', 'from_serial', 'category_id'].includes(name)) {
      ...
      if (quantity && category_id) {
        try {
          ...
          if (serials.length === 0) setSerialError("No serial numbers found.");
          else if (serials.length < parseInt(quantity)) setSerialError(`Only ${serials.length} products available, but you requested ${quantity}.`);
          else setSerialError("");
  ...
  const handleRemoveSerial = (removeIdx) => {
    setFormData(prev => {
      ...
      return { ...prev, serial_numbers: updatedSerials, quantity: updatedSerials.length };
    });
  };
    ...
    axios.post(`${API_BASE_URL}/saleStore`, formData)
      ...
      .catch(err => {
        ...
        if (err.response && err.response.status === 422) {
          ...
          else if (res.message) setErrors(prev => ({ ...prev, quantity: res.message }));
        }
      });
  };
  ...
  return (
    <div className="w-100 py-4 px-4 bg-white" style={{ minHeight: '100vh' }}>
      ...
      <Form onSubmit={handleSubmit}>
          ...
          <Col md={4}>
            <Form.Group controlId="customer_id">
              ...
              <Select
                ...
                styles={{
                  control: (base) => ({ ...base, minHeight: '32px', height: '32px', fontSize: '0.875rem', boxShadow: 'none', borderColor: '#ced4da' }),
                  valueContainer: (base) => ({ ...base, paddingTop: '0px', paddingBottom: '0px' }),
                  indicatorsContainer: (base) => ({ ...base, height: '32px' }),
                  dropdownIndicator: (base) => ({ ...base, padding: '2px 6px' }),
                  clearIndicator: (base) => ({ ...base, padding: '2px 6px' }),
                  input: (base) => ({ ...base, margin: '0px', padding: '0px' }),
                  singleValue: (base) => ({ ...base, display: 'flex', alignItems: 'center' })
                }}
              />
              {formErrors['customer_id'] && <div className="text-danger small">{formErrors['customer_id']}</div>}
            </Form.Group>
          </Col>
        ...
        <Row className="mb-3 g-2">
          <Col md={4} sm={6} xs={12}>
            ...
            <Form.Control size="sm" name="quantity" value={formData.quantity} onChange={handleChange} isInvalid={!!formErrors['quantity']} placeholder="Enter Quantity" style={{ boxShadow: 'none', borderColor: '#ced4da' }} />
          ...
          <Col md={4} sm={6} xs={12}>
            ...
            <Form.Control size="sm" name="shipment_name" value={formData.shipment_name} onChange={handleChange} placeholder="Enter Shipment Name" style={{ boxShadow: 'none', borderColor: '#ced4da' }} isInvalid={!!formErrors['shipment_name']} />
            <Form.Control.Feedback type="invalid">{formErrors['shipment_name']}</Form.Control.Feedback>
          </Col>
        </Row>

        <div className='mb-3'>
          <Form.Label className="mb-2 text-dark fw-normal">Product Serial No.</Form.Label>
          <Button variant="outline-secondary" className='ms-3' onClick={handleAddClick}>Add Product</Button>
        </div>

        <div className="w-100 d-flex flex-wrap gap-2 mb-3">
          {formData.serial_numbers.map((value, idx) => (
            <div key={idx} className="d-flex align-items-center border rounded-3 px-2 py-1" style={{ width: '105px', backgroundColor: '#F8F9FA', position: 'relative' }}>
              <Form.Control type="text" size="sm" value={value} onChange={(e) => handleSerialChange(idx, e.target.value)} className="shadow-none bg-transparent border-0 p-0 flex-grow-1" style={{ minWidth: 0 }} />
              {formErrors[`serial-${idx}`] && <div className="text-danger small mt-1 ms-1">{formErrors[`serial-${idx}`]}</div>}
              <Button variant="link" size="sm" onClick={() => handleRemoveSerial(idx)} style={{ color: '#dc3545', textDecoration: 'none', fontWeight: 'bold', fontSize: '0.8rem', marginLeft: '2px', padding: 0 }}>×</Button>
            </div>
          ))}
        </div>
        ...
        <Row className="mb-3">
          <Col md={4}>
            ...
            <Form.Control size="sm" type="date" name="shipment_date" value={formData.shipment_date} onChange={handleChange} style={{ boxShadow: 'none', borderColor: '#ced4da' }} isInvalid={!!formErrors['shipment_date']} />
            <Form.Control.Feedback type="invalid">{formErrors['shipment_date']}</Form.Control.Feedback>
          </Col>
          <Col md={4}>
            <Form.Label>Delivery Date</Form.Label>
            <Form.Control size="sm" type="date" name="delivery_date" value={formData.delivery_date} onChange={handleChange} style={{ boxShadow: 'none', borderColor: '#ced4da' }} isInvalid={!!formErrors['delivery_date']} />
            <Form.Control.Feedback type="invalid">{formErrors['delivery_date']}</Form.Control.Feedback>
          </Col>
          <Col md={4}>
            <Form.Label>Tracking No.</Form.Label>
            <Form.Control size="sm" name="tracking_no" value={formData.tracking_no} onChange={handleChange} style={{ boxShadow: 'none', borderColor: '#ced4da' }} isInvalid={!!formErrors['tracking_no']} />
            <Form.Control.Feedback type="invalid">{formErrors['tracking_no']}</Form.Control.Feedback>
          </Col>
        </Row>
      ...
      <ProductModal show={showModal} onHide={() => setShowModal(false)} loading={loadingProducts} products={products} onAdd={handleAddProducts} existingSerials={formData.serial_numbers} />
    </div>
  );
}
