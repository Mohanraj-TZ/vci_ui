...
export default function PurchaseSparepartsPage() {
  ...
  const fetchWarrantyDetails = useCallback(async (purchaseId) => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/sparepart-purchases/${purchaseId}`,
        {
          headers: {
      ...
      const errorMessage =
        ...
        "Failed to load warranty details. Please check your network or server configuration.";
  ...
  useEffect(() => {
    if (formData.invoiceNo && showReturnForm) {
      const selectedPurchase = spareparts.find(
        (p) => String(p.invoice_no) === String(formData.invoiceNo)
      );
      if (
        selectedPurchase &&
        ...
        const sparepartItems = selectedPurchase.items
          .map((item) =>
            availableSpareparts.find(
              (sp) => String(sp.id) === String(item.sparepart_id)
            )
          )
      ...
      setFormErrors((prevErrors) => {
        ...
        if (field === "warranty_months") {
          ...
          if (value && (isNaN(warrantyMonths) || warrantyMonths < 0)) {
            newErrors[`warranty_months-${index}`] =
              "Warranty months must be a non-negative number.";
          } else {
            delete newErrors[`warranty_months-${index}`];
          }
        }
  ...
  const handleInputChange = (e) => {
    ...
    setFormErrors((prev) => {
      ...
      if (!value) {
        newErrors[name] = `${
          name.charAt(0).toUpperCase() + name.slice(1).replace("No", " No.")
        } is required.`;
      } else {
  ...
  const validateForm = (payload, items, isReturnForm = false) => {
    ...
    if (isReturnForm) {
      ...
      if (
        ...
        errors.items =
          "Please add at least one spare part with a valid quantity.";
      } else {
        items.forEach((item, index) => {
          const returnedQuantity = parseInt(item.quantity, 10);
          const selectedPurchase = spareparts.find(
            (p) => String(p.invoice_no) === String(payload.invoice_no)
          );
          let purchasedQty = 0;
          if (selectedPurchase) {
            const purchasedItem = selectedPurchase.items.find(
              (pi) => String(pi.sparepart_id) === String(item.sparepart_id)
            );
            if (purchasedItem) {
          ...
          if (
            ...
            if (returnedQuantity > purchasedQty) {
              errors[
                `quantity-${index}`
              ] = `Return quantity (${returnedQuantity}) cannot exceed purchased quantity (${purchasedQty}).`;
            } else if (returnedQuantity > availableQty) {
              errors[
                `quantity-${index}`
              ] = `Return quantity (${returnedQuantity}) cannot exceed current available quantity (${availableQty}).`;
            }
          }
        });
      }
  ...
  const handleShowWarranty = async (purchase) => {
    ...
    try {
      ...
      toast.error(
        "An unexpected error occurred while loading warranty details."
      );
    } finally {
      setLoading(false);
    }
...
const handleFormSubmit = async e => {
    ...
    // Rest of your existing handleFormSubmit logic remains the same
    try {
        setLoading(true);
        let data;
        if (editingPurchase) {
  ...
  const handleReturnFormSubmit = async (e) => {
    ...
    try {
      if (editingReturn && editingReturn.id) {
        ...
        setReturns((prev) =>
          prev.map((r) =>
            r.id === editingReturn.id ? { ...payload, id: editingReturn.id } : r
          )
        );
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `${API_BASE_URL}/sparepart-purchases/${id}`,
        {
          headers: { Accept: "application/json" },
        }
      );
      if (data?.success) {
        const updatedSpareparts = spareparts.filter(
  ...
  const filteredSpareparts = spareparts.filter(
    (purchase) =>
      getVendorNameById(purchase.vendor_id)
        .toLowerCase()
    ...
    <div className="px-4" style={{ fontSize: "0.75rem" }}>
      ...
      <Card className="border-0 shadow-sm rounded-3 p-2 px-4 mt-2 bg-white">
        ...
        <div className="table-responsive">
          <table
            ref={tableRef}
            className="custom-table table align-middle mb-0"
          >
              ...
              <div className="d-flex justify-content-between align-items-center mb-2">
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "6px",
                    cursor:
                      sparePartsRows.length >= invoiceSpareparts.length
                        ? "not-allowed"
                        : "pointer",
                    outline: "none",
                    boxShadow: "none",
          ...
          {loading ? (
            ...
            <div>
              <p>
                ...
                {selectedWarranty.invoice_no || "N/A"}
              </p>
              <p>
                <strong>Vendor:</strong>{" "}
                {getVendorNameById(selectedWarranty.vendor_id) || "N/A"}
              </p>
              <p>
                <strong>Purchase Warranty Start:</strong>{" "}
                {selectedWarranty.warranty_start_date
                  ? new Date(
                      selectedWarranty.warranty_start_date
                    ).toLocaleDateString("en-GB")
                  : "N/A"}
              ...
              <p>
                <strong>Purchase Warranty End:</strong>{" "}
              ...
              <p>
                <strong>Purchase Warranty Status:</strong>
                {selectedWarranty.warranty_status ? (
                  ...
                  >
                    {selectedWarranty.warranty_status.charAt(0).toUpperCase() +
                      selectedWarranty.warranty_status.slice(1)}
                  </span>
                ) : (
                  "N/A"
                )}
            ...
            <p>
              No warranty details available. Please try again or contact
              support.
            </p>
          )}
        </Modal.Body>
