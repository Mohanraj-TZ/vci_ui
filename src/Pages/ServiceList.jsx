...
      status,
      created_by: 2, // Replace with logged-in user id
      items: repairForms,
    });
    onHide();
  };
...
                isInvalid={!!errors.challanNo} // ✅ Added invalid state
                required 
              />
              <Form.Control.Feedback type="invalid">{errors.challanNo}</Form.Control.Feedback>
            </Col>
            {/* ✅ Replaced Form.Control with Form.Select for places */}
            <Col>
...
    axios
      .get(`${API_BASE_URL}/spareparts`) // Assuming a new endpoint to fetch components
      .then((res) => {
...
const handleShowTrackingModal = (item) => {
    // Navigate to a new route that accepts the serviceVciId as a URL parameter
...
    // Fetch only the VCI items with issues for the specific serviceVciId
    axios
...
      .catch((err) => {
        const errorMessage =
          err.response?.data?.message || "Failed to create repair challan.";
